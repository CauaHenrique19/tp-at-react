...
const subtitleSleep = (data: any, t: (key: string) => string) => {
  ...
  return `${duration} ${t("min")}., ${t("from")} ${start_hour.format(
    "HH:mm"
  )} ${t("to")} ${end_hour.format("HH:mm")}`;
};

const subtitleEat = (data: any, t: (key: string) => string) => {
  const hour = dayjs(adjustDateTimeForTimezone(data.start_date)).format(
    "HH:mm"
  );
  if (data.type === 1) {
